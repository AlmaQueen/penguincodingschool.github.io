...
window.onload=function() {
  ...
  game.items = [{price: 100, description: "Dank Dagger", id: 0},
  {price: 300, description: "Yogi Bear", id: 1},
  {price: 150, description: "Esla's Evil Soul", id: 2},
  {price: 404, description: "ERROR 404", id: 3}]
...
function setMaps1() {
  ...
  for (var i=0; i<foregroundData1.length; i++) {
    ...
    for(var j=0; j<foregroundData1[0].length;j++) {
      if (foregroundData1[i][j] == 2 || foregroundData1[i][j] == 4 ||foregroundData1[i][j] == 15 ||foregroundData1[i][j] === 0 || foregroundData1[i][j]==3) {
        var collision = 1;
      collisionData[i][j] = collision;
    }
  }
...
player.visibleItems = [];
player.itemSurface = new Surface(game.itemSpriteSheetWidth,
...
var greeter = {
  action: function() {
    npc.say("A Path Has Been Opened And Still Your Hairline!");
    for(var i = 0; i<player.inventory.length; i++) {
      if(player.inventory[i] === 3) {var chess = true;}
    }
    if(chess) {setMaps1();setStage1();}
    else {npc.say("Your Hairline has an Error")}
  }
};
...
var setBattle = function(){
  ...
  battle.hitStrength = function(hit){
    ...
    for(var i =0; i< player.inventory.length; i++){
      if (player.inventory[i] === 0){var ax = true;}
      else if (player.inventory[i] === 2){var ice = true;}
      else if (player.inventory[i] === 3){var chess = true;}
      }
      
      if (ax){return Math.round ((Math.random() +1.5)*hit);}
      else if (ice){return Math.round ((Math.random()*2)*hit);}
  ...
  currentEnemy.hp = currentEnemy.hp - playerHit;
  battle.menu.text = "Your hit did " + playerHit + "damage! The enemy is at " + currentEnemy.hp;
    if (currentEnemy.hp <= 0){
      battle.won()
    }
}
  ...
  else {battle.menu.text = "You don't know any magic spells";}
}},
...
var setShopping = function(){
    ...
    shop.greeting = "Whats up I got the goods if you got the dough.";
    shop.apology = "U R BROKE!!!!";
    shop.sale = "Enjoy you little toy";
    shop.farewell = "See You Around";
